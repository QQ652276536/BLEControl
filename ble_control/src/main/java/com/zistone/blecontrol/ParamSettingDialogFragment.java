...
public class ParamSettingDialogFragment extends DialogFragment implements View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    private static MyProgressDialogUtil.ConfirmListener _confirmListener;
    ...
    public static ParamSettingDialogFragment NewInstance(String[] strArray, MyProgressDialogUtil.ConfirmListener listener) {
        ...
        switch (v.getId()) {
            ...
            case R.id.btn2_paramsetting: {
                ...
                Log.i(TAG, String.format("发送参数设置(Bit):\n门检测开关用采用常开型(关门开路)%s\n门锁检测开定于关用采用常开型(锁上开路)" +
                        "%s\n正常开锁不告警%s\n有外电可以进入维护方式%s\n启用软关机%s\n不检测强磁%s\n使用低磁检测阀值%s\n启用DEBUG软串口%s", bitStr1, bitStr2, bitStr3, bitStr4, bitStr5,
                        bitStr6, bitStr7, bitStr8));
                ...
                _confirmListener.OnConfirm(data);
            }
            break;
        }
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        Log.i(TAG, String.format("收到查询到的参数(Bit):\n门检测开关(关门开路)%s\n锁检测开关(锁上开路)%s\n正常开锁不告警%s\n有外电可以进入维护方式%s\n启用软关机%s\n不检测强磁%s\n使用低磁检测阀值%s\n启用DEBUG" +
                "软串口%s", bitStr1, bitStr2, bitStr3, bitStr4, bitStr5, bitStr6, bitStr7, bitStr8));
        if (bitStr8.equals("1"))
