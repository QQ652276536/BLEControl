...
public class PowerActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    //蓝牙锁状态
    private static final String CMD_3A_STATE = "68000000000000683A0100E316";
    //蓝牙锁名称
    private static final String CMD_3A_NAME = "68000000000000683A0101E316";
    ...
    private static final int RECEIVE_BLE_LOCK = 40;
    ...
    private TextView _txt2, _txt5, _txt6, _txt7, _txt8, _txtVersion, _txtHumidity, _txtSmoke, _txtWater, _txtMagnetic1, _txtMagnetic2,
            _txtMagnetic3, _txtBridge, _txtElecState, _txtWindow, _txtOpen, _txtOffline, _txtSupply, _txtLockName, _txtCycle;
    private StringBuffer _stringBuffer = new StringBuffer();
    private MyProgressDialogUtil.ConfirmListener _confirmListener;
    ...
    private TimerTask _refreshTask = new TimerTask() {
        ...
        public void run() {
            try {
                ...
                //先查蓝牙锁名称，查到了再查蓝牙锁状态
                if (!_isSearchLockName) {
                    Thread.sleep(100);
                    MyBleUtil.SendComm(CMD_3A_NAME);
                    Log.i(TAG, "发送'蓝牙锁名称'指令：" + CMD_3A_NAME);
                } else {
                    Thread.sleep(100);
                    MyBleUtil.SendComm(CMD_3A_STATE);
                    Log.i(TAG, "发送'蓝牙锁状态'指令：" + CMD_3A_STATE);
                }
    ...
    private boolean _isSearchLockName = false;
            ...
            switch (message.what) {
                ...
                case RECEIVE_BASEINFO: {
                    ...
                    if (versionStr.contains("V")) {
                        ...
                        _powerActivity._txtVersion.setTextColor(Color.DKGRAY);
                    } else {
                        _powerActivity._txtVersion.setText("未连接");
                        _powerActivity._txtVersion.setTextColor(Color.RED);
                    }
                ...
                break;
                //蓝牙锁状态
                case RECEIVE_BLE_LOCK: {
                    //蓝牙锁名称
                    if (!_powerActivity._isSearchLockName) {
                        String name1 = MyConvertUtil.HexStrToStr(strArray[10] + strArray[11] + strArray[12] + strArray[13]);
                        String name2 = strArray[14] + strArray[15] + strArray[16] + strArray[17];
                        String logStr = "蓝牙锁名称\n" + cmdStr + "\n名称：" + name1 + name2 + "\n";
                        Log.i(TAG, logStr);
                        if ((name1 + name2).contains("_")) {
                            _powerActivity._txtLockName.setText(name1 + name2);
                            _powerActivity._txtLockName.setTextColor(Color.DKGRAY);
                            _powerActivity._isSearchLockName = true;
                        } else {
                            _powerActivity._txtLockName.setText("未连接");
                            _powerActivity._txtLockName.setTextColor(Color.RED);
                        }
                    }
                    //蓝牙锁状态
                    else {
                        //唤醒周期
                        int cycle = Integer.parseInt(strArray[10] + strArray[11], 16);
                        _powerActivity._txtCycle.setText(cycle + "h");
                        //保留
                        int what = Integer.parseInt(strArray[12] + strArray[13], 16);
                        //锁状态
                        byte[] stateBytes = MyConvertUtil.HexStrToByteArray(strArray[14] + strArray[15]);
                        String bitStr = MyConvertUtil.ByteToBitBig(stateBytes[1]);
                        int b1 = Integer.parseInt(String.valueOf(bitStr.charAt(0)));
                        int b2 = Integer.parseInt(String.valueOf(bitStr.charAt(1)));
                        int b3 = Integer.parseInt(String.valueOf(bitStr.charAt(2)));
                        int b4 = Integer.parseInt(String.valueOf(bitStr.charAt(3)));
                        int b5 = Integer.parseInt(String.valueOf(bitStr.charAt(4)));
                        int b6 = Integer.parseInt(String.valueOf(bitStr.charAt(5)));
                        int b7 = Integer.parseInt(String.valueOf(bitStr.charAt(6)));
                        int b8 = Integer.parseInt(String.valueOf(bitStr.charAt(7)));
                        if (b2 == 0) {
                            _powerActivity._txtBridge.setText("正常");
                            _powerActivity._txtBridge.setTextColor(Color.GREEN);
                        } else {
                            _powerActivity._txtBridge.setText("异常");
                            _powerActivity._txtBridge.setTextColor(Color.RED);
                        }
                        if (b3 == 0) {
                            _powerActivity._txtElecState.setText("正常");
                            _powerActivity._txtElecState.setTextColor(Color.GREEN);
                        } else {
                            _powerActivity._txtElecState.setText("异常");
                            _powerActivity._txtElecState.setTextColor(Color.RED);
                        }
                        if (b4 == 0) {
                            _powerActivity._txtWindow.setText("正常");
                            _powerActivity._txtWindow.setTextColor(Color.GREEN);
                        } else {
                            _powerActivity._txtWindow.setText("异常");
                            _powerActivity._txtWindow.setTextColor(Color.RED);
                        }
                        if (b5 == 0) {
                            _powerActivity._txtOpen.setText("关闭");
                            _powerActivity._txtOpen.setTextColor(Color.GREEN);
                        } else {
                            _powerActivity._txtOpen.setText("打开");
                            _powerActivity._txtOpen.setTextColor(Color.RED);
                        }
                        if (b6 == 0) {
                            _powerActivity._txtOffline.setText("在线");
                            _powerActivity._txtOffline.setTextColor(Color.GREEN);
                        } else {
                            _powerActivity._txtOffline.setText("离线");
                            _powerActivity._txtOffline.setTextColor(Color.RED);
                        }
                        if ("00".equals(b7 + "" + b8) || "0".equals(b7 + "" + b8)) {
                            _powerActivity._txtSupply.setText("电池供电");
                        } else if ("01".equals(b7 + "" + b8)) {
                            _powerActivity._txtSupply.setText("无线供电");
                        } else if ("10".equals(b7 + "" + b8)) {
                            _powerActivity._txtSupply.setText("有线供电");
                        }
                        //锁标识
                        int flag = Integer.parseInt(strArray[16], 16);
                        //校验码
                        int crc = Integer.parseInt(strArray[17], 16);
                        Log.i(TAG, String.format("蓝牙锁状态（Bit）：\n锁梁%d\n电池状态%d\n供电窗口%d\n开闭锁状态%d\n锁具离线%d\n供电状态%s\n", b2, b3, b4, b5, b6, b7 + "" + b8));
                    }
                }
                break;
                //综合测试
                case RECEIVE_TESTA: {
                    ...
                    String bitStr = MyConvertUtil.ByteToBitBig(bytes1[0]);
                ...
                case RECEIVE_OPENDOORS3: {
                    ...
                    String bitStr = MyConvertUtil.ByteToBitBig(bytes[0]);
    ...
    private void Resolve(String data) {
         ...
         * 蓝牙锁
         * */
        else if (strArray[8].equals(("BA"))) {
            what = RECEIVE_BLE_LOCK;
        }
        /*
         * 开关门协议
         *
    ...
    private void InitListener() {
        ...
        _confirmListener = new MyProgressDialogUtil.ConfirmListener() {
            @Override
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        _txtBridge = findViewById(R.id.txt_lock_bridge);
        _txtElecState = findViewById(R.id.txt_lock_elec);
        _txtWindow = findViewById(R.id.txt_lock_window);
        _txtOpen = findViewById(R.id.txt_lock_open);
        _txtOffline = findViewById(R.id.txt_lock_offline);
        _txtSupply = findViewById(R.id.txt_lock_supply);
        _txtLockName = findViewById(R.id.txt_lock_name);
        _txtCycle = findViewById(R.id.txt_lock_cycle);
